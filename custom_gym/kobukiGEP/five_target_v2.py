...
class FiveTargetEnv_v2(gym.Env):
    ...
    def __init__(self, idx=0):
        ...
        room_x = 3.8
        room_y = 2.6
        self.max_pos_x = room_x / 2.
        self.min_pos_x = self.max_pos_x * -1.
        self.max_pos_y = room_y / 2.
        self.min_pos_y = self.max_pos_y * -1.
        self.num_targets = 5
        ...
        self.high_state = np.array([self.max_pos_x, self.max_pos_y, 1, 1])
        ...
        self.target_coord = [(np.cos(x) * target_radius, np.sin(x) * target_radius) for x in self.target_coord]
        '''# Concate mid target
        mid_targets = [0, 180]
    ...
    def step(self, action):
        ...
        if self.id == 0 or self.id == 1: # goal traj trainingTarget [nan, nan, random, random]
            vec = np.array([xpos-self.trainingTarget[2], ypos-self.trainingTarget[3]])
            dist = np.linalg.norm(vec)
            if dist < self.target_size + self.agent_size:
                done = True
                print('Success')
        ...
        # hit the wall
        if not done:
            if xpos - self.agent_size <= self.min_pos_x or xpos + self.agent_size >= self.max_pos_x or ypos - self.agent_size <= self.min_pos_y or ypos + self.agent_size >= self.max_pos_y:
    ...
    def reset(self, task = None, max_timesteps = 50):
        ...
        self.state = np.array([0, -1.125, 0, 1])
