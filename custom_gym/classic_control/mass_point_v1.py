...
class MassPointEnv_v1(gym.Env):
    ...
    def __init__(self, idx=0):
        ...
        self.high_obs = np.array([np.inf] * 4)
        self.low_obs = np.array([-np.inf] * 4)
    ...
    def step(self, action):
        ...
        p1, p2 = np.array([xpos, ypos]), np.array(self.task)
        reward += -np.linalg.norm(p1 - p2) * 0.05
        if self.Dedug: print(reward)
        ...
        if xpos == 1 or xpos == -1 or ypos == 1 or ypos == -1:
            ...
            self.done = True
            reward += -1
            if self.Dedug: print('Hit the Wall')
        ...
        if not self.done and self.timesteps >= self.max_timesteps:
            self.done = True
            reward += -0.5
            if self.Dedug: print('Times Up')

        return self.get_obs(), reward, self.done, {'hit':hit, 't':self.timesteps, 'face': [xface, yface]}

    def reset(self, task=None):
        # Task
        self.task = np.random.uniform(0, 1, (2,)) if task is None else np.array(task)
    ...
    def get_obs(self):
        obs = np.concatenate([self.state[0:2], self.task])
